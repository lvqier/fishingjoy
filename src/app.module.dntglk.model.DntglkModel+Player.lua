...
slot3 = "dntglk.fishConfig.Dntglk_BulletSet"

requireLuaFromModule("dntglk.model.DntglkModel+Cannon")

slot4 = {}
slot1 = class("dntglk.model.DntglkModel+Cannon", "Player")
...
slot1.ctor = function (slot0, slot1)
	...
	slot0.__queuedLockFishId = 0
	slot0.lockingIndex = 0
	slot0.fixLockingIndex = false
	slot4 = slot0
...
slot1.ClearSet = function (slot0, slot1)
	...
	slot0.m_Wastage = 0
	slot0.m_nCannonType = Dntglk_BulletSet.BulletSet[1].CannonType
	slot0.BulletCount = 0
...
slot1.SetUpdateCannon = function (slot0)
	...
	slot0.UpdateCannonConfig(slot2)
end
...
slot1.UpdateCannonConfig = function (slot0)
	slot1 = nil
	slot4 = slot0
	...
	if slot0.GetCannonSetType(slot3) == 0 then
		...
		if slot0.GetCannonMultiply(slot3) >= 6 then
			slot1 = Dntglk_CannonSet.CannonSet[1].CannonType[3]
		else
			slot4 = slot0

			if slot0.GetCannonMultiply(slot3) >= 3 then
				slot1 = Dntglk_CannonSet.CannonSet[1].CannonType[2]
			else
				slot1 = Dntglk_CannonSet.CannonSet[1].CannonType[1]
			end
		end
	else
		slot4 = slot0
		...
		if slot0.GetCannonSetType(slot3) == 1 then
			...
			if slot0.GetCannonMultiply(slot3) >= 6 then
				slot1 = Dntglk_CannonSet.CannonSet[1].CannonType[6]
			else
				slot4 = slot0
				slot0._cannonConfig = ((slot0.GetCannonMultiply(slot3) < 3 or Dntglk_CannonSet.CannonSet[1].CannonType[5]) and Dntglk_CannonSet.CannonSet[1].CannonType[4]) or Dntglk_CannonSet.CannonSet[1].CannonType[1]
...
slot1.GetCannonConfig = function (slot0)
	...
	if slot0.GetCannonSetType(slot3) == 0 then
		...
		if slot0.GetCannonSetType(slot3) == 1 then
			...
			if slot0.GetCannonMultiply(slot3) >= 6 then
				...
				slot4 = slot0
				slot1 = (slot0.GetCannonMultiply(slot3) < 3 or Dntglk_CannonSet.CannonSet[1].CannonType[5]) and Dntglk_CannonSet.CannonSet[1].CannonType[4]
			end
		end
	end
	...
	return Dntglk_CannonSet.CannonSet[1].CannonType[1]
end
