...
slot0.setIsIdle = function (slot0, slot1)
	slot0.isIdle = slot1
end

slot0.setIsFreezing = function (slot0, slot1)
	slot0.isFreezing = slot1
end

slot0.GetType = function (slot0)
	return DNTGLK_OBJ_TYPE.EOT_FISH
end
...
slot0.ExecuteEffects = function (slot0, slot1, slot2, slot3, slot4)
	...
	if slot0.GetState(slot8) < DNTGLK_OBJ_STATE.EOS_DEAD then
		slot10 = slot0
...
slot0.SetPosition = function (slot0, slot1, slot2)
	...
	if slot1 < -80 or slot2 < -80 or slot1 > 1520 or slot2 > 980 then
		if slot0.m_bEntered == true and not slot0.useMultiPointsLocking then
		end
	else
		slot0.m_bEntered = true
	end
end
...
slot0.AddBoundingBox = function (slot0, slot1, slot2, slot3)
	...
	slot8 = math.abs(slot9) + slot3
	slot0.m_fMaxRadio = math.max(#slot0.bbList + 1, slot0.m_fMaxRadio)
	slot0.bbList.size = #slot0.bbList
end
	...
	if slot0.m_nRedTime > 0 then
		...
		if slot0.m_nRedTime == 0 then
			...
			for slot5, slot6 in slot2(slot3) do
				if slot6.target and not slot6.target._shadow then
					slot9 = slot6.target
					slot14 = 255
...
slot0.OnHit = function (slot0)
	...
	for slot4, slot5 in slot1(slot2) do
		if slot5.target and not slot5.target._shadow then
			slot8 = slot5.target
			slot13 = 209
