LoginAccountResetCcsView = class("LoginAccountResetCcsView")

LoginAccountResetCcsView.onCreationComplete = function (slot0)
...
LoginAccountResetCcsView.onBtnClick = function (slot0, slot1, slot2)
	if slot1 == slot0.view.btnClose then
		slot0.model:setIsAccountResetShowing(false)
	elseif slot1 == slot0.view.btnConfirm then
		slot0.controller:requestResetPw(slot0._inputPw:getText(), slot0._inputPwConfirm:getText())
	end
end

LoginAccountResetCcsView.onZoomShowTweenComplete = function (slot0)
	ZoomPopUpChildView.onZoomShowTweenComplete(slot0)
	slot0._inputPw:show()
	slot0._inputPwConfirm:show()
end

LoginAccountResetCcsView.hide = function (slot0, slot1, slot2)
...
LoginAccountResetCcsView.destroy = function (slot0)
