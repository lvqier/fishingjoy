LhdBalanceExXQLItemCcsView = class("LhdBalanceExXQLItemCcsView")

LhdBalanceExXQLItemCcsView.onCreationComplete = function (slot0)
	ClassUtil.extends(slot0, CcsListViewItem)
	slot0.bg:setSpriteFrame("lhd_balance2_middle_solid.png")
end

LhdBalanceExXQLItemCcsView.updateView = function (slot0)
	if slot0._data then
		for slot4 = 1, 12, 1 do
			...
			if slot0._data[slot6] ~= nil and slot0._data[slot6][slot7] and slot0._data[slot6][slot7] ~= 0 then
				...
				slot5:setSpriteFrame(slot0.controller:getBalancePointTextureXQL(slot0._data[slot6][slot7]))
			else
...
LhdBalanceExXQLItemCcsView.destroy = function (slot0)
