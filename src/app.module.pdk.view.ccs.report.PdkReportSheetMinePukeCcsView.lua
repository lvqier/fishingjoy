slot2 = "PdkReportSheetMinePukeCcsView"
PdkReportSheetMinePukeCcsView = class(slot1)

PdkReportSheetMinePukeCcsView.onCreationComplete = function (slot0)
	slot5 = slot0
...
PdkReportSheetMinePukeCcsView.initPukeNode = function (slot0)
	...
	slot0._outPukeNodes = {}
	slot0._pukeNodes = {}
	slot0._outPukeCsbPath = "common/PdkOutPuke.csb"
	slot0._pukeCsbPath = "common/PdkPuke.csb"
...
PdkReportSheetMinePukeCcsView.onCurIdxChanged = function (slot0)
	slot3 = slot0.model
	...
	if slot0.model.getCurIdx(slot2) == -1 then
		...
		slot0.resetHandAndOutNodeSize(slot0.model.getReportUsersInfoDic(slot3)[PDK_VIEW_CHAIRID_ME].handcard, slot0, table.nums(1))

		slot7 = false

		slot0.outSheetPukeNode.setVisible(slot0.model.getReportUsersInfoDic(slot3)[PDK_VIEW_CHAIRID_ME].handcard, slot0.outSheetPukeNode)

		slot7 = false

		slot0.buchu.setVisible(slot0.model.getReportUsersInfoDic(slot3)[PDK_VIEW_CHAIRID_ME].handcard, slot0.buchu)
	...
	elseif slot1 >= 0 then
		...
		if slot1 < table.nums(slot0.model.getReportStepDatasDic(slot5)) then
			...
			if slot0.model.getReportStepDatasDic(slot3)[slot1].chairid ~= slot0.model._viewChairIdDic[PDK_VIEW_CHAIRID_ME] then
				slot7 = slot0.model
...
PdkReportSheetMinePukeCcsView.resetHandAndOutNodeSize = function (slot0, slot1, slot2)
