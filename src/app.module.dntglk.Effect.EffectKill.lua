...
slot1.Execute = function (slot0, slot1, slot2, slot3, slot4, slot5)
	...
	if DNTGLK_IS_ANDROID_PLATFORM then
	...
	for slot16, slot17 in pairs(slot1) do
		...
		if slot17.GetId(slot19) ~= slot9 then
			...
			if slot17.GetState(slot19) < DNTGLK_OBJ_STATE.EOS_DEAD then
				if slot8 == 0 then
					slot20 = slot17
					...
					if slot17.InSideScreen(slot19) then
						...
						if slot17.GetMoveCompent(slot19).HasBeginMove(slot19) then
							slot24 = DNTGLK_OVER_BY_EFFECT
							slot18, slot19 = slot17.ExecuteEffects(slot19, slot17, slot2, slot3, slot4)
							slot6 = slot6 + slot18

							if not DNTGLK_IS_ANDROID_PLATFORM and DNTGLK_OVER_BY_BULLET == slot5 then
								slot24 = {
					...
					if slot17.InSideScreen(slot19) then
						...
						if slot17.GetMoveCompent(slot19).HasBeginMove(slot19) then
							...
							if slot0.GetParam(slot19, slot0) == slot17.GetTypeID(slot0) then
								...
								if slot17.GetFishType(slot19) == DNTGLK_SPECIAL_FISH_TYPE.NORMAL then
									slot24 = DNTGLK_OVER_BY_EFFECT
									slot18, slot19 = slot17.ExecuteEffects(slot19, slot17, slot2, slot3, slot4)
									slot6 = slot6 + slot18
				...
				elseif slot8 == 3 then
					...
					if slot17.GetRefershID(slot19) == slot11 then
						...
						if slot17.InSideScreen(slot19) then
							...
							if slot17.GetMoveCompent(slot19).HasBeginMove(slot19) then
								slot18 = slot5
								slot25 = (slot12 < 200 or slot12 >= 300 or DNTGLK_OVER_BY_BULLET) and slot5
								slot19, slot20 = slot17.ExecuteEffects(slot20, slot17, slot2, slot3, slot4)
								slot6 = slot6 + slot19

								if not DNTGLK_IS_ANDROID_PLATFORM and DNTGLK_OVER_BY_BULLET == slot18 then
									slot25 = {
							...
							slot21 = DNTGLK_OBJ_STATE.EOS_DEAD
