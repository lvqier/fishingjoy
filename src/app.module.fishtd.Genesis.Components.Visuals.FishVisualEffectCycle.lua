slot0 = class_C("FishVisualEffectCycle", ClassLoader:aquireClass("FishVisual"))
...
slot0.loadVisualFish = function (slot0, slot1)
	...
	slot0._cycleSpine = sp.SkeletonAnimation:create(slot7 .. ".json", slot7 .. ".atlas", slot1.Effect.Scale)

	slot0._cycleSpine:setPosition3D({
		x = slot1.Effect.OffsetX,
		y = slot1.Effect.OffsetY,
		z = slot1.Effect.OffsetZ
	})
	...
	slot0._visualNode:addChild(slot0._cycleSpine, 1)
end
...
slot0.changeDeadAction = function (slot0)
	...
	slot0._cycleSpine:runAction(cc.Sequence:create(cc.DelayTime:create(slot3), cc.FadeTo:create(0.3, 0)))
end
